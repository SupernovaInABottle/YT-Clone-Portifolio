...
def channel(ch_id):
    ...
    # If the content_id is not a completed broadcast or a short then append it to the completed_broadcasts list
    for vid_id in ch_clive_response['items']:
        if vid_id['id']['videoId'] not in ch_video_response['items'][0]['id']['videoId'] and not is_short(vid_id['id']['videoId']):
            completed_broadcasts.append(vid_id['id']['videoId'])

    next_page_token = ch_video_response.get('nextPageToken')
    if not next_page_token:
        pass

    for content_id in completed_broadcasts:
        video_data_request = youtube.videos().list(
            part='snippet',
            id=content_id
        )
